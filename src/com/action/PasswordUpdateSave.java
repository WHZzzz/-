...
public class PasswordUpdateSave extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		{
			//查询原密码是否正确
			if (new UserDao().CheckPassword(session.getAttribute("id").toString(), Password)) {
				//修改密码
				UserBean cnbean=new UserBean();
				cnbean=new UserDao().GetBean(Integer.parseInt(session.getAttribute("id").toString()));
				cnbean.setuser_pass(Password2);
		...
		else if(type.equals("2"))//实验室管理员身份
		{
			//查询原密码是否正确
			if (new UserDao().CheckPassword(session.getAttribute("id").toString(), Password)) {
				//修改密码
