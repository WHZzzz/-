...
public class EquipmentAddSave extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		//查询是否存在
		List<EquipmentBean> list=new EquipmentDao().GetList("Equipment_Name='"+Equipment_Name+"' and Equipment_LabID="+Equipment_LabID, "");
		if(list.size()>0)
		{
			out.print("<script language='javascript'>alert('该实验室中已经存在该设备！');history.back(-1);</script>");
			out.flush();out.close();return null;
		}
		//添加
		...
		new EquipmentDao().Add(cnbean);
		    
		//跳转
		out.print("<script language='javascript'>alert('添加成功！');window.location='EquipmentManager.action';</script>");
		out.flush();out.close();return null;
