...
public class EquipFixUpdateSave extends ActionSupport{
	...
	private String fix_fac ;
	 ...
	 public String getEquipment_LabID() {
		...
		public void setEquipment_LabID(String equipment_LabID) {
			Equipment_LabID = equipment_LabID;
		}
	...
	public String execute() throws Exception {
		...
		//查询名称是否存在
				List<EquipFixBean> list=new EquipFixDao().GetList("fix_name='"+fix_name+"' and fix_id!="+fix_id, "");
				if(list.size()>0)
				{
		...
		cnbean=new EquipFixDao().GetBean(Integer.parseInt(fix_id));
		...
		new EquipFixDao().Update(cnbean);
		    
		//跳转
		out.print("<script language='javascript'>alert('修改成功！');window.location='EquipFixManager.action';</script>");
		out.flush();out.close();return null;
